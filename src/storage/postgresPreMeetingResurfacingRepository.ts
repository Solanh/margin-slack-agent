...
const PREPARE_SQL = `
  ...
  JOIN LATERAL (
    SELECT prior.id
    FROM meetings prior
    WHERE prior.workspace_id = upcoming.workspace_id
      AND prior.user_id = upcoming.user_id
      AND prior.series_key = upcoming.series_key
      AND prior.starts_at < upcoming.starts_at
      AND EXISTS (
        SELECT 1
        FROM notes n
        WHERE n.meeting_id = prior.id
    ...
    LIMIT 1
  ) prior_choice ON TRUE
  LEFT JOIN user_notification_preferences p
    ON p.workspace_id = upcoming.workspace_id
   AND p.user_id = upcoming.user_id
  LEFT JOIN meeting_series_preferences sp
    ON sp.workspace_id = upcoming.workspace_id
   AND sp.user_id = upcoming.user_id
   AND sp.series_key = upcoming.series_key
  WHERE upcoming.id = $2
    AND upcoming.workspace_id = $3
    AND upcoming.user_id = $4
    AND upcoming.provider = 'google_calendar'
    AND upcoming.series_key = $5
    AND COALESCE(p.resurfacing_enabled, TRUE) = TRUE
...
const CLAIM_DUE_SQL = `
  ...
  SELECT
    c.*,
    upcoming.title AS upcoming_meeting_title,
    upcoming.starts_at AS upcoming_starts_at
  FROM claimed c
  JOIN meetings upcoming
    ON upcoming.id = c.upcoming_meeting_id
   AND upcoming.workspace_id = c.workspace_id
   AND upcoming.user_id = c.user_id
...
const GET_NOTES_SQL = `
  SELECT
    n.id,
    prior.id AS prior_meeting_id,
    prior.title AS prior_meeting_title,
    prior.starts_at AS prior_meeting_starts_at,
    n.note_type,
    n.status,
    n.priority,
  ...
  FROM pre_meeting_resurfacings r
  JOIN meetings prior
    ON prior.id = r.prior_meeting_id
   AND prior.workspace_id = r.workspace_id
...
const RESOLVE_INCLUDED_SQL = `
  ...
  WHERE r.id = $1
    AND r.workspace_id = $2
    AND r.user_id = $3
    AND n.meeting_id = r.prior_meeting_id
    AND n.workspace_id = r.workspace_id
    AND n.user_id = r.user_id
    AND n.status = 'open'
    AND n.note_type IN ('action', 'question')
